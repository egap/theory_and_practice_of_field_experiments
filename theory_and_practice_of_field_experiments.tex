...
\usepackage{hyperref}
\hypersetup{
            pdftitle={The Theory and Practice of Field Experiments: An Introduction from the EGAP Learning Days},
            pdfauthor={Jake Bowers, Maarten Voors, and Nahomi Ichino},
            colorlinks=true,
            linkcolor=Maroon,
            filecolor=Maroon,
            citecolor=Blue,
...
\title{The Theory and Practice of Field Experiments: An Introduction from the EGAP Learning Days}
\author{Jake Bowers\footnote{\url{https://jakebowers.org}}, Maarten Voors\footnote{\url{https://sites.google.com/site/maartenvoors/}}, and Nahomi Ichino\footnote{\url{https://nahomi.github.io/}}}
\date{March 03, 2021}
...
The book includes \textbf{slides} on the core content, the \href{researchdesignform.html}{EGAP research design form}, and \textbf{references} to research examples and slides used in previous Learning Days. This material builds significantly on and links to EGAP's work on methodology, summarized in the \href{https://egap.org/methods-guides/}{EGAP Methods Guides}. We have also expanded on the material that is usually covered during the Learning Days workshops, with new modules on the research design process, measurement, and ethical considerations, and significant extensions to the materials on hypothesis testing, estimation, and statistical power. The slides and modules presented here contain too much information to be covered in a single week. We have chosen to present more rather than less information, however, to aid instructors tailoring their course for their specific audiences.
...
The materials included in this book have been developed over the past years by various Learning Days instructors. These include (in alphabetical order) Jake Bowers, Jasper Cooper, Ana De la O, Lindsay Dolan, Natalia Garbiras Díaz, Macartan Humphreys, Nahomi Ichino, Salif Jaiteh, Gareth Nellis, Dan Nielson, Rafael Piñeiro, Fernando Rosenblatt, Tara Slough, Peter van der Windt and Maarten Voors. We thank Natalia Garbiras Díaz, Macartan Humphreys, and Tara Slough in particular for comments on an early draft of the book.

At EGAP, tremendous support has been provided by Matt Lisiecki, Ingrid Lee, Goldie Negelev, Max Mendez-Back and others. Learning Days have been generously funded by the Hewlett Foundation and supported by institutions around the world including African School of Economics (Benin), Universidad Diego Portales (Chile), Universidad de los Andes (Colombia), Ghana Center for Democratic Development (Ghana), Mercy Corps (Guatemala), Invest in Knowledge (Malawi), NYU Abu Dhabi (UAE), Universidad Católica del Uruguay (Uruguay).
...
\begin{itemize}
\item
  What makes a \textbf{good research question}? A good research question advances science and/or is a question the answer to which will change a policy decision.
\item
...
\item
  The questions we ask arise, often implicitly, from our values and from our understanding about how the world works. These \textbf{theories} structure our questions --- make the questions relevant. And the experiments that we execute teach us about the theory --- we hope that the evidence and data arising from these research designs improves our understanding.
\item
  What are the \textbf{core components of a research design}?
\item
...
\item
  \textbf{Pre-registration}: what is it? why should we do it? how should we do it?
\end{itemize}
...
\begin{itemize}
\item
  \href{https://egap.github.io/learningdays-resources/Slides/Examples/research_design_2-benin.pdf}{Design Talk from EGAP Learning Days at African School of Economics, Benin, March 2018}
\item
...
\item
  \autocite[Chapter 12]{gerber_field_2012} Includes some examples of experimental research designs.
\end{itemize}
...
\item
  ...
  \item
    \textbf{Cluster}: assign groups or clusters of observations to the same treatment condition.
  \end{itemize}
\item
...
\item
  \autocite[Chapter 2 on randomization]{glennerster_running_2013}
\item
  \autocite[Chapter 2: Causal Inference and Experimentation]{gerber_field_2012}
\end{itemize}
...
We cannot directly observe causal effects because of the \emph{fundamental problem of counterfactual causal inference} (\href{causal-inference.html}{causal inference module}); we cannot observe \emph{both} how a person or a community responds in a treatment condition \emph{and} how it responds in a control condition. Then how can we learn about these \emph{unobserved causal effects} using what we observe? In a randomized experiment, we can assess \emph{guesses} or \emph{hypotheses} about the unobserved causal effects by comparing what we observe in a given experiment to what we would observe if we were able to repeat the experimental manipulation and the guess or hypothesis were true.
...
\item
  ...
  \end{itemize}
\item
  Be careful when testing \textbf{many hypotheses} (for example, if you have more than two treatment arms or if you are assessing the effects of a treatment on multiple outcomes). We should be careful to \textbf{adjust the \(p\)-values or confidence intervals} to reflect the number of tests/intervals produced.
\end{itemize}
...
\begin{itemize}
\item
  \autocite[Chapter 3: Sampling Distributions, Statistical Inference, and Hypothesis Testing]{gerber_field_2012}
\item
  \autocite[Chapter 2: Causal Inference in Randomized Experiments]{rosenbaum2010design}
\item
  \autocite[Part I: Randomized Experiments]{rosenbaum2017observation}
\end{itemize}
...
In this module, we introduce several types of estimands, the target quantity to be estimated. The choice of estimand is a scientific decision -- what quantity is useful for us to learn about? And as part of the research design, we want to select an appropriate estimator for this estimand. We discuss how estimators are applied to data to generate an estimate of our estimand and how to characterize the variability of this estimate.
...
\item
  An \textbf{estimator} is a recipe for calculating a guess about the value of an estimand. For example, the difference between means of observed outcomes for \(m\) treated units and for \(N-m\) untreated units is one estimator of \(\bar{\tau}\).
\item
...
\item
  \textbf{Analyze as you randomize} in the context of estimation means that (1) our standard errors should measure variability from randomization and (2) our estimators should target estimands defined in terms of potential outcomes.
\item
...
To estimate effects and test hypotheses, we often measure an outcome of interest with quantitative data derived from surveys, behavioral games, or administrative records. For causal questions, we typically use data on immediate and final outcomes and core mechanisms. We use baseline data to identify relevant subgroups, adjust our estimates or help block-randomize our treatment. Data can be noisy (random error) and/or biased (systematic error). Measurements should be valid and reliable.
...
\begin{itemize}
\item
  When we represent some attribute of a unit by some number, letter, word, symbol in some systematic way (perhaps in a cell in a simple dataset), we are \textbf{measuring}.
\item
...
\item
  Invalid measurement can make it hard for your \href{https://egap.github.io/learningdays-resources/Exercises/design-form.Rmd}{research design} to effectively counter
...
\item
  We have said ``Analyze as you randomize'' elsewhere (\href{estimands-and-estimators.html}{estimands and estimators}). Remember that you randomized treatment \textbf{assignment}, not whether the treatment is received or whether a unit participates in data collection.
\item
  \textbf{Missing data on the outcome (attrition)} is especially a problem if the patterns of missingness are caused by the treatment itself. This is the most common problem.
  ...
  \end{itemize}
\item
  \textbf{Non-compliance}. The effect of treatment assignment is not the same as the effect of receiving the treatment. Sometimes units will not comply with their assigned treatment status, but not all hope is lost.
...
\item
  \href{https://github.com/acoppock/Green-Lab-SOP}{Standard operating procedures for Don Green's lab at Columbia University}. A comprehensive set of procedures and rules of thumb for conducting experimental studies.
\item
  \autocite[Chapters 5--8]{gerber_field_2012} These chapters address non-compliance, attrition, and interference.
\end{itemize}
...
\emph{A randomized experiment involves one group of humans changing the lives of another group humans.} When one person influences the life of another, the influencer has responsibilities not to harm the person being influenced. Those working in government do this as a matter of course --- their very job is to provide food, shelter, safety, justice, etc., to their people. Academics, whose work does not usually have immediate impacts on the public, must remember to also carefully consider how the intervention might change the lives of those exposed to it, as well as those not exposed.
...
\begin{itemize}
\item
  Research must weigh the \textbf{potential benefits} of the knowledge to be gained from the research against the \textbf{potential harms} to human subjects.
\item
...
\item
  \textbf{Informed consent}: Can you ensure that research subjects have the freedom to refuse to participate and/or drop out of the study if they want to?
\item
...
\item
  \autocite{Evans2021ethics} \href{https://www.cgdev.org/sites/default/files/WP565-Evans-Ethical-issues-and-RCTs.pdf}{pdf}
\end{itemize}
...
\tightlist
\item
  \textbf{Potential outcome \(Y_i(T)\)} What outcome \(Y\) that unit \(i\) \emph{would}
...
\item
  ...
  outcomes under treatment and control, \(Y_i(1)-Y_i(0)\), for unit \(i\). See the module on \href{causal-inference.html}{causal inference}.
\item
...
\item
  ...
  data you have. An example of an estimator is the difference-in-means. See the module on \href{estimands-and-estimators.html}{estimands and estimators}.
...
\item
  \textbf{\(p\)-value} The probability of seeing a test statistic as large (in absolute value) as or larger than the test statistic calculated from observed data (for example, the difference in means). See the module on \href{hypothesis-testing.html}{hypothesis testing}.
\item
...
\item
  ...
  \tightlist
  \item
    \textbf{Sampling distribution} The distribution of estimates (e.g., estimates of
  ...
  \item
    \textbf{Standard error} The standard deviation of the sampling distribution. A
    bigger standard error means that our estimates are more susceptible to
    sampling variation. See the module on \href{estimands-and-estimators.html}{estimands and
    estimators}.
  \end{itemize}
\item
...
\item
  ...
  become ever nearer the true value of the estimand as the sample size increases is a \emph{consistent estimator} of that estimand. A consistent estimator may or may not be unbiased. See the module on \href{estimands-and-estimators.html}{estimands and estimators}.
\item
...
\item
  \textbf{Attrition} When outcomes for some subjects are not measured. Example:
...
\item
  \textbf{Encouragement design} An experiment that randomizes \(Z\) (treatment
...
RStudio can be freely downloaded from the RStudio website, \url{https://www.rstudio.com/products/rstudio/download/}. In the table, click the blue \texttt{Download} button at the top of the left column, ``RStudio Desktop Open Source License'' as depicted below in Figure 1. Once you select this button, the page will jump to a list of download options as depicted in Figure 2.
...
If successfully downloaded, your Console will resemble Figure 4, except that the urls will differ depending on your location.
...
In order to write and save reproducible code, we will open a fourth panel, the Editor, by clicking on the icon with a white page with a plus sign on the upper-left corner of the RStudio interface and selecting \texttt{R\ Script}, as depicted in Figure 5.
...
Beyond basic functions, we can also store values, data, and functions in the global environment. To assign a value to a variable, use the \texttt{\textless{}-} operator. All stored values, functions, and data will appear in the environment tab in the Accounting panel. In Figure 7, we define the variable \texttt{t} to take the value \(3 \times \frac{6}{14}\), and can see that it is stored under Values.
...
R provides a suite of tools, ranging from built-in plot functions to packages to graph data, models, estimates, etc. The final Miscellaneous panel allows for the quick viewing of graphs in RStudio. Figure 8 shows a plot in this panel. Leaning Days will discuss how to plot data; for now, don't worry about the graphing the code in the Editor.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{setwd}\NormalTok{(}\StringTok{"~TaraLyn/Dropbox/EGAP Learning Days Admin/Workshop 2018_2 (Uruguay)/"}\NormalTok{)   }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(a <-}\StringTok{ }\DecValTok{5}\NormalTok{)     }\CommentTok{# "<-"  is the assignment command; it is used to}
\end{Highlighting}
\end{Shaded}
